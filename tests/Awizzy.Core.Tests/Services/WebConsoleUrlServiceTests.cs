...
public class WebConsoleUrlServiceTests
{
    ...
    [Test]
    public async Task BuildConsoleUrlAsync_SendsCredentialsAsSessionJson()
    {
        ...
        await Assert.That(query).Contains("Action=getSigninToken");
        var sessionParam = System.Web.HttpUtility.ParseQueryString(query)["Session"];
        using var doc = JsonDocument.Parse(sessionParam!);
